...
<head>
<meta charset="UTF-8">
<title>FavBooks | Follow's Check</title>
...
<body>
	...
	<div id="main">
			<s:iterator value="session.frCheck">
				<s:form action="FrCheckAction">
					<table id="book">
					<tbody>
						<tr>
							<td id="book-img">
						...
						<s:if test="session.alReadCheck == 'off'">
							<a href='<s:url action="FrAlReadAction" />' id="read" >Favorite!</a>
						</s:if>
						<s:if test="session.alReadCheck == 'on'">
							<a href='<s:url action="FrOffReadAction" />' id="read-off" >Cancel Favorite</a>
						</s:if>
						<a href='<s:url action="BackFollowersInfoAction" />' id="check-btn" >Back</a>
					</td>
				</tr>
				</tbody>
			</table>
