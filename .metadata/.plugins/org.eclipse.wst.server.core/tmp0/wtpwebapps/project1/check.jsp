...
<body>
	...
	<div id="main">
			<s:iterator value="session.check">
				<s:form action="CheckAction">
					...
					<tbody>
						...
						<s:if test="session.alReadCheck == 'off'">
							<a href='<s:url action="AlReadAction" />' id="read" >Favorite!</a>
						</s:if>
						<s:if test="session.alReadCheck == 'on'">
							<a href='<s:url action="OffReadAction" />' id="read-off" >Cancel Favorite</a>
						</s:if>
						<a href='<s:url action="GoHomeAction" />' id="check-btn" >Back</a>
					</td>
				</tr>
				</tbody>
			</table>
